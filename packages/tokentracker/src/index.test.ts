...
describe("Tracker", () => {
  ...
  describe("constructor", () => {
    it("should throw error if apiKey is not provided", () => {
      ...
      ).toThrow("TokenTracker requires an apiKey in config");
    });

    it("should use default endpoint if not provided", () => {
      const defaultTracker = new Tracker({ apiKey: "test" });
  ...
  describe("recordEvent", () => {
    ...
    it("should default status to SUCCESS if not provided", async () => {
      vi.mocked(fetch).mockResolvedValue({
        status: 200,
        statusText: "OK",
        text: async () => JSON.stringify({ id: "123", createdAt: "2026-05-05T00:00:00Z", cost: 0.001 }),
      } as Response);
    ...
    it("should respect timeout setting", async () => {
      const shortTimeoutTracker = new Tracker({
        apiKey: mockApiKey,
        endpoint: mockEndpoint,
        timeout: 100,
      });
    ...
    it("should handle status with error message", async () => {
      vi.mocked(fetch).mockResolvedValue({
        status: 200,
        statusText: "OK",
        text: async () => JSON.stringify({ id: "123", createdAt: "2026-05-05T00:00:00Z", cost: null }),
      } as Response);
  ...
  describe("debug mode", () => {
    it("should log debug information when debug is enabled", async () => {
      ...
      const debugTracker = new Tracker({
        apiKey: mockApiKey,
        endpoint: mockEndpoint,
        debug: true,
      });
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        "[llmwatch] TokenTracker initialized",
        expect.any(Object),
      );
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("[llmwatch] Sending event"),
        expect.any(String),
      );
